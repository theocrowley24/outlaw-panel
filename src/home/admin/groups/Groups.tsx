...
import './Groups.scss';
import {UserGroup, UserGroupMapper} from "./UserGroup";
import {
    Button,
    Checkbox,
    Dialog,
...
const Groups = () => {
    ...
    const Permissions = (): any[] => {
        ...
        for (let i = 0; i < displayedPermissions.length; i++) {
            view.push(<FormControlLabel className="permission" key={i}
                                        control={<Checkbox name={displayedPermissions[i].id?.toString()} color="primary"
                                                           onChange={handleCheckbox}
                                                           checked={displayedPermissions[i].has || false}/>}
                                        label={displayedPermissions[i].name}/>)
        }
    ...
    return (
        <div>
            <div className="ranks-wrapper">
                <p className="sub-title">Select group<span className="material-icons m-icon"
                                                           onClick={handleOnClickNewRankDialog}>add</span></p>
                ...
                <Dialog open={openNewRankDialog} onBackdropClick={handleClose}>
                    ...
                    <DialogContent>
                        <div className="h-centre"><TextField id="standard-basic" label="Name"
                                                             onChange={e => setNewRankName(e.target.value)}/></div>
                        <div className="flex medium-gap">
                            <div className="h-gap"><Button variant="outlined" onClick={handleClose}>Cancel</Button>
                            </div>
                            <div className="h-gap"><Button variant="contained" color="primary"
                                                           onClick={handleCreateRank}>Create</Button></div>
                        </div>
                    </DialogContent>
                </Dialog>
                ...
                <Dialog open={openRenameRankDialog} onBackdropClick={handleClose}>
                    ...
                    <DialogContent>
                        <div className="h-centre"><TextField id="standard-basic" label="Name"
                                                             onChange={e => setRenameRankName(e.target.value)}/></div>
                        <div className="flex medium-gap">
                            <div className="h-gap"><Button variant="outlined" onClick={handleClose}>Cancel</Button>
                            </div>
                            <div className="h-gap"><Button variant="contained" color="primary"
                                                           onClick={handleRenameRank}>Update</Button></div>
                        </div>
                    </DialogContent>
                </Dialog>
            </div>
